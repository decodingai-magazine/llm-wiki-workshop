...
- Design the tools around how agents search and write, not around database operations — raw operations make agents struggle. [[wiki/sources/mcp-servers-for-continual-learning-via-graphrag]]
- Server-level `instructions` are a routing layer: they tell the model which tool fits which question, and they live beside the tools. [[wiki/sources/mcp-servers-for-continual-learning-via-graphrag]]
- Pair a flexible tool with a deterministic one and say when to use each — capability and predictability bought separately. [[wiki/sources/mcp-servers-for-continual-learning-via-graphrag]]
