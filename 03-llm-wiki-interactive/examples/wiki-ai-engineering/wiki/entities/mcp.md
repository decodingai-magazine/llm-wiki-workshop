...
- The server is a delivery mechanism, not a logic layer — the same business logic serves batch pipelines and live tool calls. [[wiki/sources/agentic-graphrag-via-mcp-servers]]
- Exposing the memory over MCP is what makes it reachable from any harness without a custom API layer. [[wiki/sources/retrieval-strategies]]
- The protocol surface is where write tools return in milliseconds and hand the work to a pipeline behind them. [[wiki/sources/mcp-servers-for-continual-learning-via-graphrag]]
