...
timestamp: 2026-08-31T14:36:31Z
source_count: 7
---
...
> Synthesis: the repo, its companion tool-routing note, and the four lesson articles share one author and one founding citation (LangChain's Terminal-Bench result — swapping only the harness under a fixed model moved it from ~30th place to the top 5). [[wiki/sources/article-building-a-coding-agent-from-scratch-system-design]], [[wiki/sources/article-the-coding-agent-loop]], [[wiki/sources/article-run-coding-agents-safely]], [[wiki/sources/article-context-engineering-for-coding-agents]], [[wiki/repos/github-decodingai-magazine-building-a-coding-agent-from-scratch-course/ARCHITECTURE]] That number appears in five of this page's seven sources but is one data point cited five times, not five independent tests. Six of the seven sources — the repo's `ARCHITECTURE.md`, its new `tool-call-routing-to-the-permission-gate` note (same codebase, same commit), and the four Substack lessons — trace to one author and one project (`decode`/Paul Iusztin); DSP's talk notes remain the only source here that reaches the "harness is where behavior lives" conclusion from a genuinely separate line of evidence (MCP/connectivity architecture, not Terminal-Bench).
...
- Behind one policy layer, `decode` ships two structurally different human-in-the-loop mechanisms depending on which of the harness's two entry points is used. In the TUI, an `ASK` decision suspends the running turn task itself on an in-process, ephemeral `asyncio.Future` owned by a single `DecisionChannel` — nothing is persisted, a process crash loses it outright, and the channel captures the app's one input surface until resolved. Under `decode run --hitl`, `agent/loop.py`'s whole boundary/queue/`DecisionChannel` machinery is bypassed entirely — the Kitaru adapter intercepts `ApprovalRequired` directly — and resolves `write`/`edit`/`bash` through its own durable approval checkpoint while `ask_user`/`exit_plan_mode` go through a genuine flow-scope `wait_for_input` that can outlive the process. [[wiki/repos/github-decodingai-magazine-building-a-coding-agent-from-scratch-course/tool-call-routing-to-the-permission-gate]]
...
- **[[wiki/concepts/permission-gate]]**: the gate's allow/deny/ask policy is one seam shared by both entry points, but what happens after an `ASK` is not — the harness routes it to an ephemeral in-process wait in the TUI or a durable, out-of-band Kitaru checkpoint under `--hitl`, bypassing the gate's own resolver for file/bash tools in the latter case. [[wiki/repos/github-decodingai-magazine-building-a-coding-agent-from-scratch-course/tool-call-routing-to-the-permission-gate]]
...
> Synthesis: the wiki now has one genuinely independent voice (DSP, arguing from MCP/connectivity architecture) and one heavily self-citing voice — six of seven sources, now including the new tool-routing note, all tracing to the `decode` project/author — converging on the same conclusion by different roads, which makes the conclusion more credible than any single repetition of the ~30th-to-top-5 statistic would. The new note adds a datapoint the earlier six didn't have: even within one self-citing project, the harness is not one uniform thing — the same `PermissionGate` policy is wrapped by two incompatible waiting-room implementations depending on entry mode, direct evidence for DSP's separately-sourced claim that harness choice, not the model or the policy underneath it, is what actually shapes agent behavior.
