...
aliases: ["orchestration", "orchestrator placement", "programmatic tool calling"]
sources:
...
Whether the orchestrator itself belongs inside the MCP server (exposed as a single tool to any client) or on the MCP client side (with the server exposing only raw tools and prompts) is presented as a genuinely open, unresolved question — even by someone who has implemented both. [[wiki/sources/the-right-way-of-building-agents-with-mcp-servers]]
...
- **[[wiki/entities/claude-code]]**: a pre-built orchestrator option, as an alternative to writing a custom client-side one. [[wiki/entities/claude-code]]
- **[[wiki/concepts/agent-memory]]**: memory tools and the prompts that guide them are exactly the kind of steps a custom orchestrator has to sequence. [[wiki/concepts/agent-memory]]
